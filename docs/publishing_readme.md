...
它不参与提示词拼接和生图，不会移动、改名、删除或复制原始图片。

## 1. 初始化

```powershell
uv run python -m tags_machine_core publish init G:\ai_publish
```
...
```yaml
schema: tags-machine-core.publish-workspace/v1
...
- `schema`：工作区配置版本，当前固定为 `tags-machine-core.publish-workspace/v1`。
...
```powershell
uv run python -m tags_machine_core publish classify G:\ai_publish
```
...
```powershell
uv run python -m tags_machine_core publish export G:\ai_publish --log-level info
```
...
```powershell
uv run python -m tags_machine_core publish export `
  G:\ai_publish `
  --exporter neev `
