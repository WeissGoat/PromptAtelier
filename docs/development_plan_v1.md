...
- 给动作、画风节点补结构化字段。
- 编写旧 `tags.txt` 辅助迁移脚本。当前已支持旧画风 `tags.txt` 到 style `node.yaml`、旧动作 `tags.txt` 到 action `meta.yaml`、旧背景 `tags.txt` 到 background `meta.yaml` 的迁移；character 是否需要迁移工具，后续根据真实素材形态确认。
