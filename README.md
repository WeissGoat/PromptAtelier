...
真实生图统一通过 `execution.py`：CLI、JSON API 和未来 worker 先拿到 `RenderRequest`，再进入 execution 层。`execute_render_request()` 负责 `execute-render-request` 的后端分发；当前默认只允许 NovelAI，并委托 `execute_novelai_generation()` 创建 `NovelAIClient`、保存图片、收集 PNG 内嵌参数、记录最终 request body，最后返回 `GenerationResult`。ComfyUI / SD 的真实执行仍需要显式实验开关。
