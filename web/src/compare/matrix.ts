...
export function compareDimensions(groups: Record<NodeRole, RoleNodeGroup>): Record<NodeRole, number> {
  return {
...
export function compareCount(groups: Record<NodeRole, RoleNodeGroup>): number {
  ...
  return dimensions.artist * dimensions.character * dimensions.action;
}
  ...
  for (const artist of factor(groups.artist)) {
    for (const character of factor(groups.character)) {
      for (const action of factor(groups.action)) {
        combinations.push({
          combinationId: [artist?.slotId ?? "artist-null", character?.slotId ?? "character-null", action?.slotId ?? "action-null"].join("::"),
          artist,
          character,
          action,
        });
